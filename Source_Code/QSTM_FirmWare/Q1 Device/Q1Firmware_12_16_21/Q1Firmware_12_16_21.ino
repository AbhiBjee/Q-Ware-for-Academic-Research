...
/*// Force Sensor (Left) Calibration Matrix - UL190808 (Q1-3.100)
const double FCL11 = 24.7300, FCL12 =  0.0285, FCL13 =  -0.1251;
const double FCL21 =  0.6109, FCL22 = 24.5171, FCL23 = -0.1372;
const double FCL31 =  0.1552, FCL32 =  0.7503, FCL33 = 50.1579;*/

/*// Force Sensor (Left) Calibration Matrix - UL190719 (Q1-2.100)
const double FCL11 = 25.9221, FCL12 =  -0.1411, FCL13 =  -1.1171;
const double FCL21 =  0.4843, FCL22 = 24.2977, FCL23 = 0.1504;
const double FCL31 =  0.0653, FCL32 =  0.6901, FCL33 = 49.9839;*/

// Force Sensor (Left) Calibration Matrix - UL190807 (Q1-1.100)
const double FCL11 = 24.5762, FCL12 = 0.4451,  FCL13 = 0.3361;
const double FCL21 =  0.1559, FCL22 = 24.7492, FCL23 = -0.3645;
const double FCL31 = 0.1623, FCL32 = 0.4203,  FCL33 = 49.8625;
...
void loop() {
    ...
    if (calibFlag == 1){// one time Calibration.
      Serial.println(F("Calibrating Q1..."));
      ...
      Serial.println(F("Q1 Calibration-Done"));
      //RGB_OFF();// Led off

      Serial.println(F("Q1 Device Ready for treatment."));
      DeviceStart();
      ...
      //FlexiTimer2::start();
      RGB_Color(256,256,256);// Led White 
      RGB_Color(0,0,256);               
    }
...
//CPU Restart
    if(!Serial){
      ...
      for(int ComEnd =0;ComEnd<=10;ComEnd++){
        RGB_Color(256,256,256);
        RGB_OFF();
        RGB_Color(256,0,0);//Red
        delay(50);
        RGB_Color(256,256,256);
        RGB_OFF();
        RGB_Color(0,0,256);//Blue         
        delay(50);             
      }
      CPU_RESTART
      delay(1000);
...
void QSTMprint(){  
      ...
      if(resetFlag<0){
        ...
        if ((int)(elapsTime)%2 == 0){RGB_Color(256,256,256);}
        else if ((int)(elapsTime)%2 == 1){RGB_Color(256,0,256);}
...
void RGB_Color(int redVal, int greenVal, int blueVal){
  ...
  analogWrite(redPin,(256 - redVal));
  analogWrite(greenPin,(256 - greenVal));
  analogWrite(bluePin,(256 - blueVal));
}
