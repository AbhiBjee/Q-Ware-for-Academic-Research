...
volatile double printString[18], imuParam[9];
...
//double wt_lt_volt_err = 0.02, wt_rt_volt_err = 0.02; //For Q2 v2k21b.100
double wt_lt_volt_err = 0.015, wt_rt_volt_err = 0.015; //For Q2 v2k22a.100
...
void loop() {
    ...
    if (calibFlag == 1){// one time Calibration.
      ...
      FlexiTimer2::start();
      RGB_Color(255,255,255);// Led White 
      RGB_Color(0,0,255);               
    }
    ...
    if ((unsigned long)(currentMicros - previousMicros) >= readForceInterval) {
      ...
      if (ICM_found){
        ...
        /*//Conversion to rads
        icm_Yaw = icm_Yaw*0.0174533f;//1 Degree = 0.0174533 rads
        icm_Pitch = icm_Pitch*0.0174533f;
        icm_Roll = icm_Roll*0.0174533f;*/

        /*eulerAng = computeEulerAngles(quat_w, quat_x, quat_y, quat_z);

        icm_Yaw = *(eulerAng+0);
        icm_Pitch = *(eulerAng+1);
        icm_Roll = *(eulerAng+2);*/
      }
      ...
      // Force transformed in Left Load cell (UL190809)
      ...
      //FHRz = (FCR31*vXRavg)+(FCR32*vYRavg)+(FCR33*vZRavg);

      /*//Force Scaling to Exact Newtons.
      if(FHRx>0.0){FHRx=1.0*FHRx;}// Fx Positive
      if(FHRx<0.0){FHRx=1.0*FHRx;}// Fx Negative  
      if(FHRy>0.0){FHRx=1.0*FHRx;}// Fy Positive
      if(FHRy>0.0){FHRx=1.0*FHRx;}// Fy Negative
...
void QSTMprint(){  
      ...
      if(resetFlag<0){
        ...
        if ((int)(elapsTime)%2 == 0){RGB_Color(255,255,255);}
        else if ((int)(elapsTime)%2 == 1){RGB_Color(255,0,255);}
...
void ResetFlashActivated(){
  
  //delay(100);
  //RGB_Color(255,255,255);

  currentMillis=millis();
  if ((unsigned long)(currentMillis - startMillis)% 10 == 0) {
      //buttonVal = digitalRead(buttonPin);
      //if (lastButtonVal<buttonVal){buttonCount++;}
      //previousMillis = millis();
      elapsTime = (double)(previousMillis - startMillis)/1000.0;
      Serial.print("I:Q2_");
      Serial.print(elapsTime);
      Serial.print(", No Data ");
      RGB_Color(255,0,255);
      Serial.println("Reset Interrupt Activated ");
  }
  else if((unsigned long)(currentMillis - startMillis)% 10 == 5){
      RGB_Color(255,0,255);
