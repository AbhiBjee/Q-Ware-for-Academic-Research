...
import QSTMGraphicalAnalysisQ2 as qstmGA
...
#-----------------------Q2 visualization Class---------------------

class Q2_Visual_GUI(object):
    ...
    def setupUI (self, Frame):
        ...
        #win.resize(1200,700)
        Frame.setWindowTitle('QSTM Q2 Visualization')
        ...
        ###-----------------------Setup Timer for Q2 Data Read and Write-----------------------

        self.Q2timer = QtCore.QTimer()
        self.Q2timer.timeout.connect(self.UpdateExec)
    ...
    def SystemInitQ2(self, path):
        ...
        rstCnt = 0;self.tableParams =[] 
        self.RT_time=[]
        self.startTime=0.0;self.rstStrtTime=0.0;self.rstStopTime=0.0
        self.saveTime = 0.0
        ...
        self.ptr =0;self.cptr=0;self.StrPtr=1
        self.RST = 0; self.lastRST = 0
        self.path = path; self.val=0;
        self.size = self.csvFileSize(self.path)
        self.DevState = 0; self.lastDevState=-1; self.TreatSt = -1; self.fps=None;
        ...
        self.storeCSVpath=self.tempStorePath+"\\"+"_Q2RawOutputChart_"+self.Local_time+".csv"
        self.resultCSVpath=self.tempStorePath+"\\"+"_Q2ResultChart_"+self.Local_time+".csv"
        self.processCSVpath = self.processDataStorePath+"\\"+"ProcessQ2data"+".csv"
        self.rstTimeCSVpath = self.rstDataStorePath+"\\"+"RstQ2TimeData"+".csv"
        self.lstRT_PlotInterrupt = 0
        while True:            
            
            self.lstRT_PlotInterrupt = 1             
    ...
    def csvFileReadLine(self, path, size, lineNum):
        with open(path) as csvfile:            
            readCSV = csv.reader(csvfile,delimiter=',')
    ...
    def ReadDataStream(self):        
        ...
        #self.serialElements = self.csvFileReadLine(self.path, self.size, self.StrPtr)
        self.serialElements = self.DataFrame[self.StrPtr]
        #print(self.RT_PlotInterrupt, self.DevState)
        #print(self.serialElements)
        
        #self.dataString = (self.dataStream)
        #self.serialElements = self.dataString.split(",")
        self.RST = (int)(self.serialElements[12])
        
        
        if (self.RST == 0):#(self.modeState is "T"):
            self.RT_PlotInterrupt =1
            
            self.TreatButtonCnt = ((int)(self.serialElements[12]))
        
        elif (self.RST == 1):#(self.modeState is "I"):
            self.RT_PlotInterrupt =3
            

        if (self.RST>self.lastRST):
            self.DevState = 1
            
        elif(self.RST<self.lastRST):
            self.DevState = 0
...
##            print("Reset start timeStamp",(rstStrtTime))       
            ...
            if(self.TreatSt == 0):
                self.rstStrtTime = (float)(self.RST)
                self.rst = 1
                self.Ts = self.serialElements[0]           
                self.csvRow = []
                ...
                try:
                    self.csvRow = [(float)(self.serialElements[0]),self.Fx,self.Fy,self.Fz,self.Fres,self.Yg,self.Pg,self.Rg,self.Ys,self.Ps,self.Rs,self.aRms,self.rst,self.gRms,self.gX,self.gY,self.gZ,self.mX,self.mY,self.mZ]
                    self.mainCSVwriter.writerow(self.csvRow)
                    for tt in range (1,25):
                        self.Ts = (float)(self.serialElements[0])+(0.01*tt)
                        if tt<10:                            
            ...
            #storeRST_FilePath=rstDataStorePath+"\\RstQ2Chart_"+str(rstCnt)+"_"+str(round(rstStrtTime,2))+".csv"        
           ...
           # labelRow = [' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','1',' ']
            ...
            self.rstStopTime = (float)(self.RST)
...
##
    def storeData(self):
        ...
        if (self.DevState<0):           
            ...
            if (self.cptr==500):
                ...
                print("Q2 Recess{}".format(saveTime))
                #print(ptr,cptr)
            if (self.cptr>500):
        ...
        #print(Elements)
        #self.storeCSVpath=tempStorePath+"\\"+"_TempQ2Chart_"+self.Local_time+".csv"
        if (self.RT_PlotInterrupt==1) and (self.DevState==0):
            self.Ts = (float)(self.serialElements[0])
            ...
            self.Yg = (float)(self.serialElements[5])# Pg = IMU y MPU9250
            self.Pg = (float)(self.serialElements[6])# Rg = IMU x MPU9250
            self.Rg = (float)(self.serialElements[7])+90.0# Yg = IMU z MPU9250
            #self.Yg = (float)(self.serialElements[6])# Pg = IMU y MPU9250
            #self.Pg = (float)(self.serialElements[7])# Rg = IMU x MPU9250
            #self.Rg = (float)(self.serialElements[5])# Yg = IMU z MPU9250
            ...
            if (self.cptr==500):
                saveTime = float(str(self.Ts))
                print("Q2 Treat{}".format(saveTime))
                #print(ptr,cptr)
            if (self.cptr>500):
    ...
    ##            RT_time.pop(0)
            ...
            if (self.TreatSt == 0):                
                ...
                self.csvRow = [(float)(self.serialElements[0]),0.01,0.01,0.01,0.01,self.Yg,self.Pg,self.Rg,self.Ys,self.Ps,self.Rs,self.aRms,self.rst,self.gRms,self.gX,self.gY,self.gZ,self.mX,self.mY,self.mZ]
...
##
    ...
    def GetStarted(self, serialComm):
        ...
        while (True):
            ...
            if self.dataString.startswith('T:Q2'):
                self.startFlag=1
                self.Q2timer.start(1)
                break
            else:
                continue
            print (self.dataString)
    ...
    def showFinalDialog(self):
        ...
        msg.setText("End of Practise")
        msg.setInformativeText(" Do You want to close GVI ? ")
        msg.setWindowTitle("GVI Closure Message")
        msg.setDetailedText("The details are as follows:\r\n"+ "\r\n"+
                            "Wrong Device Orientation during Button Press" +
                            "Accidental button press during treatment")
        msg.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
...
##        self.StrPtr += 1
        ...
        self.now = t.time()
        if(self.StrPtr == self.size):
            self.val = self.showFinalDialog()
        elif(self.StrPtr == 1):
            if (self.waitCtr==3000):
                self.StrPtr+=1
            self.waitCtr+=1
            #print(self.waitCtr)
            return()
...
##        G_plt_max = [max(self.YawG),max(self.PitchG),max(self.RollG)]
##        G_plt_min = [min(self.YawG),min(self.PitchG),min(self.RollG)]
##        
...
##
##        if ((min(self.YawG)> -5.0) and (max(self.YawG)< 5.0)) and ((min(self.PitchG)> -5.0) and (max(self.PitchG)< 5.0)) and ((min(self.RollG)> -5.0) and (max(self.RollG)< 5.0)):
##            self.GeoAPlt.setRange(rect=None, xRange=None, yRange=(-5.0,5.0), padding=None, update=True, disableAutoRange=True)
##        else:
##            self.GeoAPlt.setRange(rect=None, xRange=None, yRange= ((min(G_plt_min)),max(G_plt_max)), padding=None, update=True, disableAutoRange=False)
...
if __name__ == "__main__":
    ...
    gui = Q2_Visual_GUI()
    gui.setupUI(Frame)
    #######path = "Output Data\Clinician_One_Q-Treatment_13-00-25_08-03-2021\_Q1RawOutputChart_13-00-25_08-03-2021.csv"# Q1 Lin Right
    #######path = "Output Data\Clinician_One_Q-Treatment_13-04-51_08-03-2021\_Q1RawOutputChart_13-04-51_08-03-2021.csv"# Q1 Lin Right
    #path = "Output Data\Clinician_One_Q-Treatment_12-44-38_08-03-2021\_Q1RawOutputChart_12-44-38_08-03-2021.csv"# Q1 Lin Left
    #path = "Output Data\Clinician_One_Q-Treatment_12-48-16_08-03-2021\_Q1RawOutputChart_12-48-16_08-03-2021.csv"# Q1 Curved Left
    #path = "Output Data\Clinician_One_Q-Treatment_12-52-53_08-03-2021\_Q2RawOutputChart_12-52-53_08-03-2021.csv"# Q2 Lin Left  
    path = "Output Data\Clinician_One_Q-Treatment_12-56-30_08-03-2021\_Q2RawOutputChart_12-56-30_08-03-2021.csv"# Q2 Curved Left
    
    
    #path = "_Q2RawOutputChart_14-10-05_09-03-2021.csv"

    #serConnect= serial.Serial(port=str("COM12"))
    #serConnect.baudrate = 115200
    gui.SystemInitQ2(path)
    #gui.GetStarted(serConnect)
    gui.Q2timer.start(10)
    Frame.show()
    sys.exit(app.exec_())
