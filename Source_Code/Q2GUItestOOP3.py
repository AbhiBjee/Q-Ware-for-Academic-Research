...
class Q2_Visual_GUI(object):
    ...
    def SystemInitQ2(self, serialComm):
        ...
        while True:
            ...
            if self.dataString.startswith('Q2 Calibration-Done'):
                ...
                #storeRST_FilePath=rstDataStorePath+"\\"+"RstQ2Chart_0_"+str(t_now)+".csv"
            ...
            #elif self.dataString.startswith('T:Q2'): #Start Device
    ...
    def ReadDataStream(self):
        ...
        if (self.serialComm.inWaiting()==0) and (self.DevState<0):
            self.dataStream= ("N:Q2_0.000,No Data")
        else:
            while(self.serialComm.inWaiting()==0):
                self.dataStream=""
                #self.dataString = str(dataStream)
        ...
        if (self.dataString.startswith('T:Q2')):#(self.modeState is "T"):
            self.RT_PlotInterrupt =1
            self.modeTimeStamp = modeElements[1]
            self.modeState,self.DeviceID = (modeElements[0]).split(":")
            self.TreatButtonCnt = ((int)(self.serialElements[12]))
        elif (self.dataString.startswith('R:Q2')):#(self.modeState is "R"):
            self.RT_PlotInterrupt =2
            self.modeTimeStamp = modeElements[1]
            self.modeState,self.DeviceID = (modeElements[0]).split(":")
            resetString = self.serialElements[1].split(" ")
            self.ResetButtonCnt = ((int)(resetString[len(resetString)-1]))
        elif (self.dataString.startswith('I:Q2')):#(self.modeState is "I"):
